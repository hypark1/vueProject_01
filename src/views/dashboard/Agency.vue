      ...
      <b-colxx xxs="12">
        ...
        <b-row class="mb-4">
          <b-colxx>
            ...
            <b-card v-if="response.agencyList">
        ...
        <b-row>
          <b-colxx class="col-lg-4 col-sm-12">
            <b-card class="mb-4 cf" title="효율 분석">
              <template v-if="resultAgency">
              <b-card class="shadow-none border">
                <b-card-text>클릭당 단가</b-card-text>
                <b-card-text style="font-size:1.8rem">{{ resultAgency.click_pay }}원</b-card-text>
              </b-card>
              <b-card class="shadow-none border mt-3">
                <b-card-text>예산 소진율</b-card-text>
                <b-card-text style="font-size:1.8rem">{{ resultAgency.pay_p }}%</b-card-text>
              </b-card>
              </template>
...
export default {
  ...
  methods: {
    ...
    showGraphBtn: function (idx) {
      ...
      var lineChart = ['View', 'Click', 'CTR', '소진예산']
    ...
    getAgency: function () {
      ...
      this.xhttp({
        url: 'report/' + this.response.user_session.cid + '/agency',
        method: 'get',
        params: send
      }).then((response) => {
        if (response.status === 200) {
          ...
          if (response.data.data === null) {
            ...
            this.response.agencyList = null
          } else {
            if (this.response.Agency !== false) {
              let data = JSON.parse(JSON.stringify(this.response.Agency.data))
              this.response.agencyList = this.response.Agency.data.list
              this.response.chart.views = []
              this.response.chart.clicks = []
              this.response.chart.ctr = []
              this.response.chart.pay = []
    ...
    respChart: function (data) {
      ...
      let data2 = this.response.Agency.data.chart
  ...
  computed: {
    resultAgency () {
      if (this.response.Agency && this.response.Agency.success === true && this.response.Agency.data) {
