<template>
  <div>
    <b-row>
      <b-colxx xxs="12">
        <piaf-breadcrumb heading="WINDOW PLAY"/>
        <div class="mb-2 mt-2">
                ...
                <b-input-group>
                  ...
                  <template v-else>
                    ...
                    <b-input-group-append>
                      <file-upload
                        class="btn btn-outline-secondary"
                        ref="windowPlay_banner"
                        v-model="File_banner"
                        name="file"
                        input-id="windowPlay_banner"
  ...
  methods: {
    ...
    submitBuilder () {
      if (this.File_bg.length > 0 &&
        ...
        } else if (!(this.File_banner.length > 0)) {
          this.$notify('error', '파일 업로드', 'Banner Image 파일을 업로드하세요.')
  ...
  watch: {
    File_bg: function (val) {
      if (val[0]) {
        ...
        this.$refs.windowPlay_bg.active = true
      }
    },
    File_banner: function (val) {
      if (val[0]) {
        this.vModel.bannerName = val[0].name
        this.$refs.windowPlay_banner.active = true
      }
    },
    File_vod: function (val) {
      if (val[0]) {
        this.vModel.vodName = val[0].name
        this.$refs.windowPlay_vod.active = true
      }
    }
  },
  computed: {
    ...
    postBanner () {
      if (this.response.vod) {
        return '/api/builder/' + this.response.builder_id + '/imgUpload/banner'
      } else {
        return ''
      }
    }
